...
public class Filmes {
	...
	public Filmes() {
		...
		if (!indexExists()) {
			System.out.println("Criando index...");
			createRating();
			...
			try {
				...
				in = new FileReader("movies_metadata.csv");

				scan = new Scanner(new File("movies_metadata.csv"));
	...
	private void createRating(){
		...
		try {
			...
			in = new FileReader("ratings_with_latlng.csv"); 

			scan = new Scanner(new File("ratings_with_latlng.csv"));
			...
			for (CSVRecord record : records) {
				...
				IndexRequestBuilder source = client.prepareIndex(INDEX, "ratings").setSource(movie);
				prepareBulk.add(source);
				bulkCount++;
				// source.get();
				if (bulkCount > 500) {
		...
		try {
			...
			XContentBuilder mapping = jsonBuilder().startObject()
					.startObject("properties")
						...
						.startObject("movieId").field("type", "long").endObject()
					.endObject()
			.endObject();

			client.admin().indices().prepareCreate(INDEX).addMapping("ratings", mapping).execute().actionGet();
	...
	public List<Map<String, Object>> getTopFilmes() {
		...
		TermsAggregationBuilder termRating = AggregationBuilders.terms("userId").field("userId").size(100);
		SearchResponse responseRating = client.prepareSearch(INDEX).setTypes("ratings").addAggregation(termRating).execute().actionGet();
		
		SearchHit[] resultsTopRating = responseRating.getHits().getHits();
		
		for (SearchHit hit : resultsTopRating) {
			System.out.println("++++++++++++++++++++++");
